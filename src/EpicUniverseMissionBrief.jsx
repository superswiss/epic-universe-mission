...
import fetchSheetJson from './lib/fetchSheet';
...
export default function EpicUniverseMissionBrief() {
  ...
  useEffect(() => {
    ...
    const gsUrl = import.meta.env.VITE_SHEET_URL;
    if (csvUrl) {
      fetchCsv(csvUrl).then(data => setSheetData(data)).catch(err => console.warn('CSV load failed', err));
      return;
    }
    const url = gsUrl || 'PASTE_WEB_APP_URL_HERE';
    if (!url || url.includes('PASTE')) return;
    fetchSheetJson(url).then(data => {
      setSheetData(data);
      console.log('Sheet data loaded', data);
    }).catch(err => console.warn('Failed to load sheet data', err));
  }, []);
    ...
    <div className="min-h-screen text-gray-900 overflow-hidden" style={{ 
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-7xl mx-auto px-8 py-12">
          {sheetData && Array.isArray(sheetData) && (
            <div style={{position:'fixed',right:16,top:84,zIndex:60,background:'#fff',padding:'6px 10px',border:'1px solid #ddd',borderRadius:6,fontSize:12}}>
